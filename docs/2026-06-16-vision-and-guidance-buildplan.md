...
- Tier-2 model: DEFAULT **Anthropic computer-use** (`claude-opus-4-8`, tool `computer_20251124`, `enable_zoom`, beta `computer-use-2025-11-24`, DIRECT to api.anthropic.com — not via OpenRouter, per the tool-expansion blocker). FALLBACK **`qwen/qwen3-vl-8b-instruct`** (OpenRouter, cheap, ScreenSpot ~92%). Select via `KAIROS_TIER2_GROUNDER=claude|qwen|off` + `KAIROS_TIER2_MODEL`. User relaxed no-Claude for this gated/rare/quality-dominated tier.
