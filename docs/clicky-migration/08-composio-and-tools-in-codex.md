...
## A0. No Cua — KAIROS IS the computer-use server

HeyClicky bundles **Cua** as a separate `computer-use` MCP server (a 22MB helper
`ClickyComputerUseRuntime`) because they did not build their own actuation. **We
do not need it and do not bundle it.** KAIROS already has its own computer-use,
built this week and live-verified:
...
**Decision: AX-first default, Cua as an ADDITIONAL MCP server when needed.**
...
Cua last-mile (browser DOM, AX-blind UI, vision). Add it the moment we hit a
real AX-blind app or serious browser automation — not required for v1 parity,
and our guide/teach layer stays ours regardless.
...
loop expose **byte-identical tools**. This is the dual-consumer invariant: while
`KAIROS_BRAIN=codex` is a permanent fallback flag (01.E), `buildActionToolset`
stays dual-consumer.
...
The MCP server MUST share the daemon process and its state. Two viable
transports; choose one in 07 (this doc recommends **HTTP-on-loopback**):

| Option | Mechanism | Register in codex config | Notes |
|---|---|---|---|
| **HTTP (recommended)** | `McpServer` over the SDK's `StreamableHTTPServerTransport`, mounted as a `/mcp` route on the EXISTING daemon `Bun.serve` at `127.0.0.1:9876` (`src/daemon/wrapApi/server.ts:68-74` already uses a routes map) — no second listener. | `[mcp_servers.kairos] url = "http://127.0.0.1:9876/mcp"` (+ `bearer_token_env_var` for the per-turn token). | In-process, shares all singletons. Easiest to debug, decouples lifecycle from the codex child. Mirrors openclicky's `127.0.0.1:32123/mcp` control bridge. |
| **stdio pair** | `McpServer` over an in-memory/stdio transport pair the `CodexBrain` hands to the codex child at spawn. | `[mcp_servers.kairos] command/args` BUT pointing at an in-daemon entrypoint that re-attaches to live state — NOT a fresh `bun run`. | What HeyClicky/codex default to, but harder to wire from Bun without re-spawning (which loses state). Only viable via a shared-memory transport. |

Recommendation: **HTTP on loopback**. KAIROS already has `Bun.serve` with a
...
JSON-RPC, `/mcp/call`, `/mcp/calls` batch, SSE `/events`). Auth: validate the
per-turn bearer token (the same per-install token from 02) on every `/mcp`
request, constant-time compared, exactly as openclicky checks its
`x-openclicky-token`.
...
| `<hot-set Composio tools>` | `retriever.getByNames(topNames)` | regex on name (`_LIST/_GET/_SEARCH/_FETCH/_READ`) | per-tool | ✅ | ✅ |
...
(not just thrown errors) and run self-heal inline:

```ts
;(globalThis as any).__kairosComposioExecute = async (name: string, args: any) => {
  toolUsage.record(name)
  const r = await composioClient.executeTool({ toolName: name, userId: composioUserId, arguments: args ?? {} })
  if (isNotConnected(r)) {                              // successful:false + NOT_CONNECTED / auth_required shape
    ...
    const healed = await selfHeal.connectAndRetry(toolkit,
      () => composioClient.executeTool({ toolName: name, userId: composioUserId, arguments: args ?? {} }))
    if (healed.status === 'connected') return healed.toolResult
    return r   // surface the original NOT_CONNECTED so Codex can tell the user to connect
  }
  return r
}
```
...
2. **`execute_tool(tool_name, arguments)`** — generic dispatch through
   ...
   decision (H) keys on the RESOLVED slug, not on `execute_tool` itself.
...
by design (warm agent, no per-call codex prompts). That **bypasses codex's own
approval** — so KAIROS's gates must be independent of codex policy. Three layers,
all daemon-side:
...
1. **MCP-server boundary confirmer.** Wrap the `execute` callback of any
   ...
   `execute_tool`, evaluate the RESOLVED slug against the destructive pattern
   (write/send/delete/create vs `_LIST/_GET/_SEARCH/_FETCH/_READ`); for a write,
   require confirmation (or queue an approval the HUD renders) before the daemon
   actually calls `sdk.tools.execute`.
...
`kairos.guide_user`, NOT bare `guide_user`. The verifier's `LOCAL_TOOLS`
exemption and the destructive `startsWith('kairos_')` checks match **bare**
names. If the ledger translator feeds namespaced names straight in, the gate
mis-fires on EVERY tool (either approval-gating local read-only tools, or
failing to recognize them). **The ledger translator in `CodexBrain` MUST strip
the `kairos__`/`kairos.` namespace before feeding the verifier and before any
`isDestructiveCall` check.** Also harden `verifier.ts:40-56` matching to be
robust to a namespace prefix as belt-and-suspenders.
...
1. **Background sub-agent / proactive WORK lane** (`codex exec --json` or a
   ...
   before touching voice. (Also re-routes `taskRunner.ts` WORK off `claude -p`
   per 05/proactive plan.)
...
- **`src/daemon/mcp/kairosMcpServer.ts` (NEW — the IN-PROCESS server factory,
  ...
  `supports_parallel_tool_calls` is honoured. Expose over
  `StreamableHTTPServerTransport` mounted as the `/mcp` route on the EXISTING
  daemon `Bun.serve` at `127.0.0.1:9876` (auth via per-turn bearer token),
  sharing live daemon state. NOT a separate process or listener.
...
- **`src/daemon/index.ts:897-900` (`__kairosComposioExecute`)** — wire inline
  ...
  wiring" comment at `index.ts:2239`.
- **`src/daemon/agents/codexBrain.ts` (NEW — see 01)** — generate the
  ...
  `isDestructiveCall` check; re-implement the destructive-stream gate (I).
- **`src/daemon/agents/loop/verifier.ts:40-56`** — make `LOCAL_TOOLS` matching
  and the `startsWith('kairos_')` destructive check robust to MCP namespacing
  (belt-and-suspenders; primary strip happens in the translator).
- **`src/daemon/wrapApi/server.ts:68-74`** — add the `/mcp` route to the existing
  `Bun.serve` routes map; bearer-token-validate every request (constant-time),
  reusing the per-install token substrate from 02.
...
- **Dual-consumer permanence** — is the in-house loop retired once Codex is
  default, or kept forever as the `KAIROS_BRAIN` fallback? Decides whether
  `buildActionToolset` stays dual-consumer indefinitely (01 treats the flag as
  permanent).
