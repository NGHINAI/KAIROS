...
final class AudioEngine {
    ...
    func start() {
        ...
        // Force the audio graph to be "valid" by connecting input → mainMixer.
        // Without a connection, AVAudioEngine on macOS may not pull data from
        // the mic. We mute mainMixer's output volume so we don't hear ourselves.
        let mixer = engine.mainMixerNode
        engine.connect(input, to: mixer, format: nativeFormat)
        mixer.outputVolume = 0
        NSLog("KAIROS audio: input → mainMixer connected, output muted")
